...
Set your environment variables as described in the Setup and Installation guide. You also need to install the `kubectl` command line tool. You can find instructions to do so in the [Google Kubernetes Engine Documentation](https://cloud.google.com/kubernetes-engine/docs/how-to/cluster-access-for-kubectl).
...
We will reference the `capital_agent` example defined on the LLM agents page.
...
[UI Testing](#ui-testing_1)[API Testing (curl)](#api-testing-curl_1)
